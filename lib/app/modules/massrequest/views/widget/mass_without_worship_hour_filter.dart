...
class MassWithoutWorshipHourFilter extends StatelessWidget {
  const MassWithoutWorshipHourFilter({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return GetBuilder<MassRequestWithWorshipController>(builder: (logic) {
      return DropdownButtonFormField<Slot?>(
        isExpanded: true,
        value: logic.selectedHour.value,
        enableFeedback: true,
        icon: const Icon(
        ...
        items: logic.selectedHours
            .map<DropdownMenuItem<Slot?>>((Slot? typeData) {
          return DropdownMenuItem<Slot?>(
            value: typeData,
            child: Text(
              logic.getTime(typeData?.startTime ?? ''),
              maxLines: 2,
              softWrap: true,
        ...
        onChanged: (Slot? value) {
          logic.updateMassTypeRepetitionHourFilter(value);
        },
      );
    });
  }
