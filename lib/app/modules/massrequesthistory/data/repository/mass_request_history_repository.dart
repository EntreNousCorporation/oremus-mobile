...
class MassRequestHistoryRepository implements IMassRequestHistoryRepository {
  ...
  Future<DataResponse<MassRequestResponse>> getMassRequests({
    ...
    Response response = await _apiClient.doRequest(
      endpoint: "/users/mass-requests?page=$page&size=${AppConstants.MASS_REQUEST_PAGING_SIZE}${(searchCriteria?.worshipPlace == null) ? '' : '&worshipPlace=${searchCriteria?.worshipPlace}'}${(searchCriteria?.typeOfMassRequest == null || searchCriteria?.typeOfMassRequest?.isEmpty == true) ? '' : '&typeOfMassRequest=${searchCriteria?.typeOfMassRequest}'}${(searchCriteria?.startDate == null || searchCriteria?.startDate?.isEmpty == true) ? '' : '&startDate=${searchCriteria?.startDate}'}${(searchCriteria?.endDate == null || searchCriteria?.endDate?.isEmpty == true) ? '' : '&endDate=${searchCriteria?.endDate}'}",
